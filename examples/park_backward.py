...
class TrajectoryGeneratorNode(Node):
    ...
    def generate_trajectory(self) -> Trajectory:
        ...
        # Trajectory segments
        L1 = 10.0  # Length of first straight (backward, accelerate to vref) [m]
        R1 = 20.0  # Radius of first turn (backward, stay at vref, brake at end) [m]
        R2 = 10.0  # Radius of second turn (forward, accelerate to vref) [m]
        L2 = 5.0  # Length of second straight (forward, stay at vref, brake at end) [m]
        ...
        for _ in range(Nnps):
            ...
            node.v = v
            node.a = a
            node.delta = math.atan((lf + lr) / R1)       # negative delta + going backward -> turning left
            node.beta = 0.0
            node.mode = 2 # backward
            node.pw_l = pwidth/2
            node.pw_r = pwidth/2
        ...
        x0 = x
        for _ in range(Nnps):
            if L2 - abs(x - x0) > xdec:  # While the remaining distance in the straight is larger than the distance to decelerate
                a = 0.0                  # stay at vref
                v = vref
            else:
                a = amin
